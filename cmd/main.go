...
import (
	...
	"github.com/lonegunmanb/terraform-null-schema/v3/schema"
	"golang.org/x/oauth2"
)

func main() {
	refreshFlag := flag.Bool("refresh", false, "Refresh null schema")
...
func checkGitHubTag(schemaVersion string) (bool, error) {
	...
	for {
		tags, resp, err := client.Repositories.ListTags(context.Background(), "lonegunmanb", "terraform-null-schema", options)
		if err != nil {
			return false, err
		}
